use super::models::{CacheFileInput, CachedFile};
...
impl CacheManager {
    ...
    pub async fn init(&self) -> Result<()> {
        ...
        std::fs::create_dir_all(&self.cache_dir).map_err(|e| {
            AppError::FileOperationFailed(format!("Failed to create cache directory: {}", e))
        })?;
    ...
    async fn run_migrations(&self) -> Result<()> {
        ...
        .await
        .map_err(|e| {
            AppError::DatabaseError(format!("Failed to create file_cache table: {}", e))
        })?;
        ...
        sqlx::query(
            "CREATE INDEX IF NOT EXISTS idx_file_cache_path ON file_cache(server_id, remote_path)",
        )
        .execute(&self.db)
        .await
        .map_err(|e| AppError::DatabaseError(format!("Failed to create index: {}", e)))?;
    ...
    pub async fn cache_file(&self, input: CacheFileInput) -> Result<PathBuf> {
        ...
        if let Some(parent) = local_path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                AppError::FileOperationFailed(format!("Failed to create cache subdirectory: {}", e))
            })?;
        }

        // Write content to local file
        std::fs::write(&local_path, &content).map_err(|e| {
            AppError::FileOperationFailed(format!("Failed to write cache file: {}", e))
        })?;
    ...
    pub async fn update_cache(
        ...
        let cached = self
            .get_cached_file(server_id, remote_path)
            .await?
            .ok_or_else(|| {
                AppError::FileOperationFailed(format!("Cache entry not found for: {}", remote_path))
            })?;

        // Write new content to local file
        std::fs::write(&cached.local_path, content).map_err(|e| {
            AppError::FileOperationFailed(format!("Failed to update cache file: {}", e))
        })?;
    ...
    pub async fn invalidate_cache(&self, server_id: &str, remote_path: &str) -> Result<()> {
        ...
        if let Some(cached) = self.get_cached_file(server_id, remote_path).await? {
            ...
            if cached.local_path.exists() {
                std::fs::remove_file(&cached.local_path).map_err(|e| {
                    AppError::FileOperationFailed(format!("Failed to delete cache file: {}", e))
                })?;
            }
        }
    ...
    pub async fn read_cached_content(
        &self,
        server_id: &str,
        remote_path: &str,
        ...
        match cached {
            Some(c) if c.local_path.exists() => {
                let content = std::fs::read(&c.local_path).map_err(|e| {
                    AppError::FileOperationFailed(format!("Failed to read cache file: {}", e))
                })?;
                Ok(Some(content))
            }
            _ => Ok(None),
    ...
    fn generate_local_path(&self, server_id: &str, remote_path: &str) -> PathBuf {
        ...
        self.cache_dir.join(server_id).join(safe_path)
    }
...
mod tests {
    ...
    async fn test_cache_file_and_retrieve() {
        ...
        let cached = manager
            .get_cached_file("test-server", "/etc/nginx/nginx.conf")
            .await
            .unwrap();
    ...
    async fn test_invalidate_cache() {
        ...
        // Invalidate cache
        manager
            .invalidate_cache("test-server", "/tmp/test.txt")
            .await
            .unwrap();
        ...
        let cached = manager
            .get_cached_file("test-server", "/tmp/test.txt")
            .await
            .unwrap();
    ...
    async fn test_update_cache() {
        ...
        manager
            .update_cache("test-server", "/tmp/update.txt", new_content)
            .await
            .unwrap();

        // Verify content is updated
        let content = manager
            .read_cached_content("test-server", "/tmp/update.txt")
            .await
            .unwrap();
        ...
        let cached = manager
            .get_cached_file("test-server", "/tmp/update.txt")
            .await
            .unwrap()
            .unwrap();
    ...
    async fn test_clear_server_cache() {
        ...
        for i in 0..3 {
            let cached = manager
                .get_cached_file("test-server", &format!("/tmp/file{}.txt", i))
                .await
                .unwrap();
            assert!(cached.is_none());
        }
